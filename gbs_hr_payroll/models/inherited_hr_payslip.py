        ...
        for run in self:
            ...
            for bank in banks:
                ...
                bank_accs = self.env['res.partner.bank'].search([('bank_id', '=', bank), ('company_id', '=', company_list)])
                                                                # ('is_payroll_account', '=', True)]) ai boolen field missing
                if not bank_accs:
    ...
    def _compute_days(self):
        for payslip in self:
            if payslip.date_from and payslip.date_to:
                start = datetime.datetime.strptime(str(payslip.date_from), "%Y-%m-%d").date()
                end = datetime.datetime.strptime(str(payslip.date_to), "%Y-%m-%d").date()
