...
from . import attendance_summary_line as asl
...
class AttendanceProcessor(models.Model):
    ...
    def get_summary_data(self, employeeId, summaryId, graceTime, operating_unit_id, att_utility_pool,
        ...
        if employee.initial_employment_date and employee.initial_employment_date:
        ...
        startDate = accPeriodResult["startDate"]
        endDate = accPeriodResult["endDate"]
        ...
        attSummaryLine = asl.TempAttendanceSummaryLine()
            ...
            while currDate <= endDate:
                ...
                # Check for Unpaid Leave
                    ...
                    attendanceDayList = att_utility_pool.getAttendanceListByAlterDay(alterDayDutyTime, day, dutyTimeMap,
                                                                                     employeeId)
                    altCurrDate = datetime.datetime.strptime(alterDayDutyTime.startDutyTime.strftime('%Y-%m-%d'),
                                                             '%Y-%m-%d')
                    attSummaryLine = self.makeDecision(attSummaryLine, attendanceDayList, altCurrDate, alterDayDutyTime,
                                                       employee, graceTime, holidayMap, empJoiningDateMap,
                                                       att_utility_pool)
                ...
                elif dutyTimeMap.get(self.getStrFromDate(
                        currDate)):  # Check this date is week end or not. If it is empty, then means this day is weekend
                    currentDaydutyTime = dutyTimeMap.get(self.getStrFromDate(currDate))
                    attendanceDayList = att_utility_pool.getAttendanceListByDay(attendance_data, currDate,
                                                                                currentDaydutyTime, day, dutyTimeMap)
                    attSummaryLine = self.makeDecision(attSummaryLine, attendanceDayList, currDate, currentDaydutyTime,
                                                       employee, graceTime, holidayMap, empJoiningDateMap,
                                                       att_utility_pool)
                else:
        ...
        if len(attSummaryLine.absent_days) > 1:
            ...
            for abs_day in attSummaryLine.absent_days:
                ...
                if consecutive and (abs_day.date == next_expected_abs):
                    attSummaryLine.absent_days.append(absent_day.TempAbsentDay(abs_day.date - day1))

        return attSummaryLine

    def makeDecision(self, attSummaryLine, attendanceDayList, currDate, currentDaydutyTime, employee, graceTime,
                     holidayMap, empJoiningDateMap, att_utility_pool):
        ...
        if attendanceDayList:
            ...
            if employee.is_executive == True:
                ...
                for i, attendanceDay in enumerate(attendanceDayList):
                    totalPresentTime = self.getDayWorkingMinutesForNonManagement(attendanceDay, currentDaydutyTime,
                                                                                 totalPresentTime)
            ...
            isLate = att_utility_pool.isLateByInTime(attendanceDayList[0].check_in,
                                                     currentDaydutyTime, graceTime)

            if isLate == True:
                attSummaryLine = self.buildLateDetails(attSummaryLine, currentDaydutyTime, currDate, totalPresentTime,
                                                       attendanceDayList)
            ...
            if empJoiningDateMap.get(employeeId):
                ...
                attSummaryLine = self.buildAbsentDetails(attSummaryLine, currentDaydutyTime, currDate, totalPresentTime,
                                                         attendanceDayList)
    ...
    def process(self, employeeIds, summaryId, operating_unit_id):
        ...
        dates = self.getAccountPeriodDate(summaryId)
    ...
    def buildLateDetails(self, attSummaryLine, currentDayDutyTime, startDate, totalPresentTime, attendanceDayList):
        ...
        attSummaryLine.late_days.append(late_day.TempLateDay(startDate, currentDayDutyTime.startDutyTime,
    ...
    def buildAbsentDetails(self, attSummaryLine, currentDayDutyTime, startDate, totalPresentTime, attendanceDayList):
        ...
        attSummaryLine.absent_days.append(absent_day.TempAbsentDay(startDate, currentDayDutyTime.startDutyTime,
                                                                   currentDayDutyTime.endDutyTime,
                                                                   currentDayDutyTime.otStartDutyTime,
                                                                   currentDayDutyTime.otEndDutyTime,
                                                                   (
    ...
    def buildWeekEnd(self, attSummaryLine, startDate):
        attSummaryLine.weekend_days.append(weekend_day.TempWeekendDay(startDate))
        return attSummaryLine
    ...
    def getDayWorkingMinutesForNonManagement(self, attendanceDay, currentDaydutyTime, totalMinute):
        ch_in = attendanceDay.check_in
        ch_out = attendanceDay.check_out
    ...
    def getDateFromStr(self, dateStr):
        if dateStr:
            return datetime.datetime.strptime(str(dateStr), "%Y-%m-%d")
        else:
