...
class HrOffenceRescheduleWizard(models.TransientModel):
    _name = 'hr.offence.reschedule.wizard'

    @api.multi
    def _get_remaining_amount(self):
        return self._context['remaining_amount']

    remaining_amount = fields.Integer(string="Remaining Amount",default =_get_remaining_amount)
    ...
    def genarate_reschudle(self):
        ...
        loan_pool = self.env['hr.employee.offence']
        loan_line_pool = self.env['hr.employee.offence.line']
        ...
        loan_amt = loan.remaining_offence_amount
