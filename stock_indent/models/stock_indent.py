...
class IndentIndent(models.Model):
    ...
    approve_date = fields.Datetime('Approve Date', readonly=True, tracking=True)
    ...
    product_lines = fields.One2many('indent.product.lines', 'indent_id', 'Products Line', readonly=True, required=True,
    ...
    approver_id = fields.Many2one('res.users', string='Approve Authority', readonly=True, help="who have approve or reject indent.", tracking=True)
    closer_id = fields.Many2one('res.users', string='Close Authority', readonly=True, help="who have close indent.", tracking=True)
    warehouse_id = fields.Many2one('stock.warehouse', 'Warehouse', readonly=True, required=True,
                                   ...
                                   states={'draft': [('readonly', False)]})
    picking_type_id = fields.Many2one('stock.picking.type', string='Picking Type', compute='_compute_default_picking_type', readonly=True, store=True)
    ...
    def _compute_default_picking_type(self):
        for indent in self:
            ...
            picking_type_ids = picking_type_obj.sudo().search([('default_location_src_id', '=', indent.warehouse_id.sudo().lot_stock_id.id), ('default_location_dest_id', '=', indent.stock_location_id.id)])
    ...
    def indent_confirm(self):
        for indent in self:
            ...
            new_seq = self.env['ir.sequence'].sudo().next_by_code('stock.indent', requested_date)
            if new_seq:
                res['name'] = new_seq
    ...
    def create_picking_and_moves(self, products):
        ...
        for line in products:
            date_planned = datetime.strptime(str(self.indent_date), DEFAULT_SERVER_DATETIME_FORMAT)
...
class IndentProductLines(models.Model):
    ...
    product_uom_qty = fields.Float('Quantity', digits='Product UoS',
                                   required=True, default=1)
    received_qty = fields.Float('Received', digits='Product UoS', help="Receive Quantity which Update by done quntity.")
    issue_qty = fields.Float('Issue Quantity', digits='Product UoS', help="Issued Quantity which Update by avilable quantity.")
    ...
    price_subtotal = fields.Float(string='Subtotal', compute='_compute_amount_subtotal', digits='Account',
                                  store=True)
    qty_available = fields.Float(string='On Hand', compute='_compute_product_qty', compute_sudo=True, help="Quantity on hand when indent issued")
    qty_available_now = fields.Float(string='In Stock Now', compute='_compute_product_qty', compute_sudo=True, help="Always updated Quantity")
