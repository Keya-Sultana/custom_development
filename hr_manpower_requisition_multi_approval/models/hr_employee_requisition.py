    ...
    def action_confirm(self):
        ...
        for requisition in self:
            if requisition.employee_id.holidays_approvers:
                ...
                self.message_post(body="You have been assigned to approve a Manpower Requisition.", partner_ids=[requisition.pending_approver.sudo(SUPERUSER_ID).user_id.partner_id.id])

    @api.multi
    def _notify_employee(self):
        for requisition in self:
            if requisition.sudo(SUPERUSER_ID).employee_id and \
                    requisition.sudo(SUPERUSER_ID).employee_id.user_id:
                self.message_post(body="Your Manpower Requisition request has been Approved.",
                                  partner_ids=[requisition.sudo(SUPERUSER_ID).employee_id.user_id.partner_id.id])
            ...
            for approver in requisition.employee_id.holidays_approvers:
                ...
                if requisition.pending_approver.id == approver.approver.id:
                    if sequence == len(requisition.employee_id.holidays_approvers):
                        ...
                        next_approver = requisition.employee_id.sudo(SUPERUSER_ID).holidays_approvers[sequence].approver
            ...
            if is_last_approbation:
                ...
                if next_approver and next_approver.id:
                    ...
                    if next_approver.sudo(SUPERUSER_ID).user_id:
                        self.suspend_security().message_post(body="You have been assigned to approve a Manpower Requisition.",
                                          partner_ids=[next_approver.sudo(SUPERUSER_ID).user_id.partner_id.id])
                requisition.suspend_security().write(vals)
