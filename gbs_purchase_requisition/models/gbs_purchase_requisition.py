...
class PurchaseRequisition(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'),
                              ...
                              ('approve_head_procurement', 'Waiting For Approval'),
                              ('done', 'Approved'),
                              ('close', 'Done'),
                              ('cancel', 'Cancelled')],
    ...
    def action_in_progress(self):
        ...
        new_seq = self.env['ir.sequence'].next_by_code('purchase.requisition', requested_date)
...
class PurchaseRequisitionLine(models.Model):
    ...
    product_ordered_qty = fields.Float('Ordered Qty', digits='Product UoS', default=1)
    name = fields.Char(related='product_id.name', string='Description', store=True)
    price_unit = fields.Float(related='product_id.standard_price', string='Unit Price', digits='Product Price', store=True)
    product_uom_id = fields.Many2one(related='product_id.uom_id', comodel_name='uom.uom', string='Product Unit of Measure', store=True)
    last_purchase_date = fields.Date(string='Last Purchase Date', compute='_get_last_purchase', store=True)
    last_qty = fields.Float(string='Last Purchase Qty', compute='_get_last_purchase', store=True)
    last_product_uom_id = fields.Many2one('uom.uom', string='Last Purchase Unit', compute='_get_last_purchase', store=True)
    ...
    def _get_product_quantity(self):
        for product in self:
            ...
            product_quant = self.env['stock.quant'].search([('product_id', '=', product.product_id.id), ('location_id', '=', location.id)])
