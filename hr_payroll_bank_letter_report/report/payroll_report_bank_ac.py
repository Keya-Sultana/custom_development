...
class PayrollReportBankAc(models.AbstractModel):
    _name = 'report.hr_payroll_bank_letter_report.report_bankletter_document'
    _description = 'Payroll Report Bank Ac'
    ...
    def _get_report_values(self, docids, data):
        ...
        for slip in slip_pool.browse(slip_ids.ids):
            payslip = {}
            payslip['emp_name'] = slip.employee_id.bank_account_id.bank_account_title
            payslip['emp_dep'] = slip.employee_id.department_id.name
            payslip['emp_desig'] = slip.employee_id.job_id.name
            ...
            for line in slip.line_ids:
                if line.code == 'NET':
                    ...
                    payslip['NET'] = formatLang(self.env,math.ceil(total_amt))
        ...
        return {
            ...
            # 'company': self.env['res.company']._company_default_get('hr_payroll_bank_letter_report').name,
