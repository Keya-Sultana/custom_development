...
class Holidays(models.Model):
    ...
    def _default_approver(self):
        ...
        if employee.sudo().holidays_approvers:
                return employee.sudo().holidays_approvers[0].approver.id
    ...
    def action_approve(self):
        for holiday in self:
            ...
            sequence = 0
            next_approver = None
            for approver in holiday.employee_id.sudo().holidays_approvers:
                sequence = sequence + 1
                if holiday.pending_approver.id == approver.approver.id:
                    if sequence == len(holiday.employee_id.sudo().holidays_approvers):
                        is_last_approbation = True
                    else:
                        next_approver = holiday.employee_id.sudo().holidays_approvers[sequence].approver
    ...
    def _compute_current_user_is_approver(self):
        if self.pending_approver.sudo().user_id.id == self.env.user.id or \
                self.pending_approver.sudo().transfer_holidays_approvals_to_user.id == self.env.user.id:
    ...
    def _compute_pending_transfered_approver_user(self):
        self.pending_transfered_approver_user = self.pending_approver.sudo().transfer_holidays_approvals_to_user
