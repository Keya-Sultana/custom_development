...
class HRShiftAlter(models.Model):
    ...
    def _compute_current_user_is_approver(self):
        if self.pending_approver.sudo().user_id.id == self.env.user.id:
    ...
    def action_approve(self):
        for alter in self:
            ...
            sequence = 0
            next_approver = None
            for approver in alter.employee_id.sudo().holidays_approvers:
                sequence = sequence + 1
                if alter.pending_approver.id == approver.approver.id:
                    if sequence == len(alter.employee_id.sudo().holidays_approvers):
                        is_last_approbation = True
                    else:
                        next_approver = alter.employee_id.sudo().holidays_approvers[sequence].approver
    ...
    def create(self, vals):
        if vals.get('employee_id', False):
            ...
            if employee and employee.sudo().holidays_approvers and employee.sudo().holidays_approvers[0]:
    ...
    def _notify_approvers(self):
        approvers = self.employee_id.sudo()._get_employee_manager()
        if not approvers:
            return True
        for approver in approvers:
            self.with_user(SUPERUSER_ID).add_follower(approver.id)
            if approver.with_user(SUPERUSER_ID).user_id:
                self.with_user(SUPERUSER_ID)._message_auto_subscribe_notify(
                    [approver.with_user(SUPERUSER_ID).user_id.partner_id.id])
        return True